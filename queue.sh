...
[[ -z ${outdir} ]] && outdir=$basedir/out.$(timestamp)
...
do
	...
	sample_name=$(basename ${sample} | \
		sed -n 's:[[:punct:]][Rr][1-2]*.fastq.gz::p;s:[[:punct:]][Rr][1-2]*.fastq::p;s:[[:punct:]][Rr][1-2]*.fq.gz::p;s:[[:punct:]][Rr][1-2]*.fq::p;s:[[:punct:]][1-2]*.fastq.gz::p;s:[[:punct:]][1-2]*.fastq::p;s:[[:punct:]][1-2]*.fq.gz::p;s:[[:punct:]][1-2]*.fq::p' )
